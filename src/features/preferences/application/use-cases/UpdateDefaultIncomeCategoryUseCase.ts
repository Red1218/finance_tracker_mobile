...
export interface UpdateDefaultIncomeCategoryRequest {
...
export class UpdateDefaultIncomeCategoryUseCase {
  ...
  public async execute(request: UpdateDefaultIncomeCategoryRequest): Promise<RepositoryResult<Preferences, Error>> {
    try {
      let incomeCatId: CategoryId | null = null;
      if (request.categoryId) {
        ...
        if (!catResult.data || catResult.data.kind !== CategoryKind.Income) {
          return Result.failure(
            new PreferencesDomainError(
              'INVALID_DEFAULT_CATEGORY',
              'Default income category must be a valid Income category.'
            )
          );
        }
        incomeCatId = catId;
      ...
      const updatedDefaults = new DefaultSettings({
        defaultExpenseCategoryId: current.defaults.defaultExpenseCategoryId,
